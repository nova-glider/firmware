...
static void MX_GPIO_Init(void) {
  ...
  /*Configure GPIO pin Output Level */
  HAL_GPIO_WritePin(GPIOA, SERVO1_Pin | SERVO2_Pin | SERVO3_Pin,
                    GPIO_PIN_RESET);

  /*Configure GPIO pin Output Level */
  HAL_GPIO_WritePin(GPIOB,
                    RFM96_DIO0_Pin | RES_RFM69_Pin | CS_SD_Pin | CS_RFM96_Pin,
                    GPIO_PIN_RESET);
...
//   // double latitude = GPSData.latitude; // latitude in degrees with decimal
//   places
//   // char latside = GPSData.latSide; // N or S
//   // double longitude = GPSData.longitude; // longitude in degrees with
//   decimal places
...
//   // int satelliteCount = GPSData.satelliteCount; // number of satellites
//   used in measurement
//   // double fix = GPSData.fix; // 1 = fix, 0 = no fix
//   // char lastMeasure[10];
//   // strcpy(lastMeasure, GPSData.lastMeasure); // hhmmss.ss UTC of last
//   successful measurement; time
...
//   float timeValue = 0.0;
//   for (int i = 0; GPSData.lastMeasure[i] && GPSData.lastMeasure[i] != '\0';
//   i++) {
//     if (GPSData.lastMeasure[i] == '.') {
//       timeValue = timeValue * 100 + (GPSData.lastMeasure[i+1] - '0') * 10 +
//       (GPSData.lastMeasure[i+2] - '0'); break;
//     }
...
//   #define ADD_STR(s) do { const char *_s = (s); for(int _i=0; _s[_i]; _i++)
//   json_buffer[pos++] = _s[_i]; } while(0)
...
//   json_buffer[pos++] = '.';
//   for(int i = 100000; i > 0; i /= 10) json_buffer[pos++] = (lat_frac / i) %
//   10 + '0';
...
//   json_buffer[pos++] = '.';
//   for(int i = 100000; i > 0; i /= 10) json_buffer[pos++] = (lon_frac / i) %
//   10 + '0';
