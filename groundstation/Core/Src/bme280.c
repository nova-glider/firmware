...
void Reset_BME280(void) {
  ...
  // If value of id register is not equal to BME280 chip id which is 0x60, wait
  // until equal to each other
...
HAL_StatusTypeDef BME280_SleepMode(void) {
  ...
  return HAL_I2C_Mem_Write(&bme_i2c, BME280_ADDR, CTRL_MEAS_REG_ADDR, 1, &init,
                           1, 1000);
}
...
/**
 * @brief  Reading temperature, pressure and humidity and calculating altitude
 * from pressure value
 *
 * @param  None
 *
...
void BME280Calculation(BME280_Data_t *result) {
  ...
  result->AltitudeP =
      44330 * (1 - pow(result->Pressure / 1013.25,
                       1 / 5.255)); /*Calculation of altitude parameter in
                                       meters with simplified atmospheric
                                       pressure formula*/

  result->AltitudeTP =
      ((log(result->Pressure / 1013.25) * (288.16) * (-8.314)) /
       (28.97 * 9.81)) *
      1000; /*Calculation of altitude parameter in
                                                                            meters
               with atmospheric pressure formula*/
}
...
void BME280Init() {
  ...
  BME280Init.Filter = FILTER_8;         // FILTER_X
  BME280Init.Mode = BME280_NORMAL_MODE; // SLEEP, NORMAL or FORCE can be written
  BME280Init.OverSampling_H = OVERSAMPLING_16; // OVERSAMPLING_X
  BME280Init.OverSampling_P = OVERSAMPLING_16; // OVERSAMPLING_X
  BME280Init.OverSampling_T = OVERSAMPLING_16; // OVERSAMPLING_X
  BME280Init.SPI_EnOrDıs =
      SPI3_W_DISABLE; // SPI3_W_DISABLE or SPI3_W_ENABLE can be written
  BME280Init.T_StandBy = T_SB_250; // T_SB_X

  uint8_t init = 0;

  // Setting it to sleep mode because the config register can only be changed
  // while the BME280 is in sleep mode
  if (BME280_SleepMode() == HAL_OK) {

    // Configuration of config register which is control standby time, filter
    // and SPI 3-wire interface
  ...
  // Configuration of ctrl_meas register which is control oversamplig of
  // Temperature-Pressure and device mode
